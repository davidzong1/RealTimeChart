...
#define queue_size 10
    ...
    public:
        dz_com(){};
        ~dz_com();
        dz_com(bool isServer, const char *IP, int PORT, bool ser_or_topic)
        {
            init(isServer, IP, PORT, ser_or_topic);
        };
        dz_com(bool isServer, const char *IP, int PORT, bool H_or_L, key_t key, bool ser_or_topic)
        {
            init(isServer, IP, PORT, H_or_L, key, ser_or_topic);
        };
        dz_com(bool H_or_L, key_t key, bool ser_or_topic)
        {
            init(H_or_L, key, ser_or_topic);
        };
