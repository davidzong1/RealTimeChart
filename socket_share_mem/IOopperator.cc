...
/**
     ...
     * @return
     */
std::string formatData(const std::string &param_name,
...
void IOoperator::write(const std::string &param_name,
                       const Eigen::MatrixXd &data)
{
    checkPath();
...
std::string IOoperator::write(std::vector<std::string> &param_names,
...
void IOoperator::readAllParams(std::vector<Eigen::VectorXd> &data,
                               std::vector<double> &time)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (iss >> firstColumn)
        {
            ...
            data.push_back(stringToVector(
                line.substr(firstColumn.length() + 1))); // 去掉第一个参数名
        }
    }
    file.close();
    return;
}
std::string IOoperator::write(std::string &filename_,
                              std::vector<SSMData> &data)
{
    checkPath();
    std::string full_path;
    int cnt = 0;
    ...
    {
        file << std::to_string(data[i].time) << " "
             << vectorToString(data[i].data) << std::endl;
    }
    file.close();
    return full_path;
}
        ...
        auto writeFiles = [&]() {
            while (true)
            {
                ...
                if (data[currentIndex].empty() ||
                    data[currentIndex][0].name.empty())
                {
                    continue;
                }
...
void IOoperator::Squeread(std::vector<std::vector<SSMData>> &data)
{
    ...
    std::thread worker([&]() {
        ...
        try
        {
            ...
            std::cout << "Uncompressing zip file...,please wait..."
                      << std::endl;
            // 解压到临时目录
            ...
            for (const auto &entry :
                 std::filesystem::directory_iterator(tempDir))
            {
                ...
                try
                {
                    ...
                    while (std::getline(file, line))
                    {
                        ...
                        try
                        {
                            ...
                            ssm.data =
                                stringToVector(line.substr(spacePos + 1));
